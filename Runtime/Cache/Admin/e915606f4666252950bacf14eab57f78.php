...
<body>
    ...
    <div class="tab-content">
      <form action="<?php echo U('save');?>" method="post" class="form-horizontal">
        <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$config): $mod = ($i % 2 );++$i;?><div class="form-item">
            <label class="item-label"><?php echo ($config["title"]); ?>
              <?php if(!empty($config["remark"])): ?><span class="check-tips">（<?php echo ($config["remark"]); ?>）</span><?php endif; ?>
            </label>
            <div class="controls">
            ...
            <?php case "5": echo hook('uploadImages', array('name' => "config[".$config['name']."]", 'value' => $config['value'] ,'tpl'=>'one')); break;?>
            <?php case "6": echo W('Editor/dateTime',array(array('time'=>$config['value'],'name'=>"config[".$config['name']."]"))); break;?>
            <?php case "7": ?><textarea id="editor" name="config[<?php echo ($config["name"]); ?>]"><?php echo ($config['value']); ?></textarea>
            <?php echo W('Editor/editor',array(array('id'=>'editor'))); break; endswitch;?>
            </div>
          </div><?php endforeach; endif; else: echo "" ;endif; ?>
        <!-- 微信公众号接口地址 -->
        <?php if(($_GET['id']) == "2"): ?><div class="form-item">
            <label class="item-label">微信接口URL<span class="check-tips">（在微信公众平台开发模式下填写以下地址）</span> </label>
            <div class="controls">
              您的微信接口URL是：<span style="color:#f00" id="token_link"><?php echo SITE_URL;?>/Wechat/index.html&amp;token=<?php echo ($token); ?></span>
  ...
  <script type="text/javascript" src="/Public/Admin/js/jquery/zclip/jquery.zclip.min.js"></script>
  <script type="text/javascript">
    $('.copy').zclip({
      path: '/Public/Admin/js/jquery/zclip/ZeroClipboard.swf',
      copy: function(){
