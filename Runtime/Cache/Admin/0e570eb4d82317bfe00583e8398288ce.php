...
<body>
  ...
  <div class="tools auth-botton">
    ...
    <button url="<?php echo U('changestatus',array('method'=>'deleteGroup'));?>" class="btn ajax-post confirm" target-form="ids" >删 除</button>
  </div>
  <!-- 数据列表 -->
  <div class="data-table table-striped">
    <table class="">
      ...
      <?php if(!empty($_list)): if(is_array($_list)): $i = 0; $__LIST__ = $_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="status_trclass_<?php echo ($vo["status"]); ?>">
            <td>
            <?php if($vo["id"] !=1 && $vo["id"] !=C('SUPPLIER_GROUP_ID') ): ?><input class="ids" type="checkbox" name="id[]" value="<?php echo ($vo["id"]); ?>" /><?php endif; ?>
            </td>
            ...
            <td class="text-center"><a href="<?php echo U('AuthManager/user?group_name='.$vo['title'].'&group_id='.$vo['id']);?>" ><?php echo ($vo["pcount"]); ?></a></td>
            <td>
            ...
            <td>
            <a href="<?php echo U('AuthManager/editgroup?id='.$vo['id']);?>">[编辑]</a>
            <?php if($vo['id'] != 1 && $vo['id'] != C('SUPPLIER_GROUP_ID')){ ?>
