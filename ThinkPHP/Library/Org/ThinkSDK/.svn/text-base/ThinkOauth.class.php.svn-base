...
abstract class ThinkOauth{
	...
	public function __construct($token = null){
		...
		$this->Type = substr(strrchr($type, '\\'), 1);
		//获取应用配置
  ...
  public static function getInstance($type, $token = null) {
  	...
  	$class = 'Org\\ThinkSDK\\sdk\\'.ucwords($name);
	...
	public function getRequestCodeURL(){
		...
		$params = array(
			...
			'redirect_uri'  =>($_SERVER['HTTPS'] != "on"? 'http://':'https://').$_SERVER['HTTP_HOST'].$this->Callback,
		...
		$return_url = $this->GetRequestCodeURL . '?' . http_build_query($params);
    //p($return_url);exit;
    return $return_url;
	}
