...
class Think extends TagLib{
    ...
    protected $tags   =  array(
        ...
        'nav'       =>  array('attr' => 'field,name,pid,postion', 'close' => 1), //获取导航
		'subnav'       =>  array('attr' => 'field,name,pid', 'close' => 1), //获取导航
    ...
    public function _nav($tag, $content){		
		...
		if(!empty($tag['position'])){
			$where .= ' AND position = '.$tag['position'];
		}

        $parse  = $parse   = '<?php ';
        $parse .= '$__NAV__ = M(\'Channel\')->field('.$field.')->where("'.$where.'")->order("sort")->select();';
        if($tree){
	 ...
	 /* 导航列表-子导航 */
    public function _subnav($tag, $content){		
        $field  = empty($tag['field']) ? 'true' : $tag['field'];
        $tree   =   empty($tag['tree'])? false : true;
